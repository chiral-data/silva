pub const APP_NAME: &str = "silva";
pub const ORG_NAME: &str = "Chiral";
pub const TAG: &str = "022";

pub const SILVA_PROJECTS_HOME: &str = "SILVA_PROJECTS_HOME";
pub const SILVA_SAKURA_ACCESS_TOKEN: &str = "SILVA_SAKURA_ACCESS_TOKEN";
pub const SILVA_SAKURA_ACCESS_TOKEN_SECRET: &str = "SILVA_SAKURA_ACCESS_TOKEN_SECRET";

pub const FILENAME_ACCOUNTS: &str = "accounts.toml";
pub const FILENAME_SETTINGS: &str = "settings.toml";
pub const FILENAME_REGISTRIES: &str = "registries.toml";
pub const FILENAME_JOBS: &str = "jobs.toml";
