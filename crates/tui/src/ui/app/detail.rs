...
pub fn render(f: &mut Frame, area: Rect, states: &mut ui::states::States, store: &data_model::Store) {
    ...
    let app_sel = super::list::get_selected(states, store).unwrap();
    let text = vec![
    ...
    if states.app.detail.list_state_pod_types.selected().is_none() {
        states.app.detail.list_state_pod_types.select(Some(0));
    }
...
fn select_pod_type(states: &mut ui::States, is_up: bool) {
    ...
    states.resource.pod_type.pod_type_sel_id = pod_type.id;
}
    ...
    match key.code {
        ...
        KeyCode::Enter => {
            ...
            if let Some(sel_idx) = list_state.selected() {
                ...
                states.resource.show_page = ui::resource::ShowPage::PodType;
                let pods_of_this_type = store.pod_mgr.pods.values()
                    .filter(|pod| pod.type_id == pod_type_sel.id)
                    .map(|sv| sv.to_owned())
                    .collect::<Vec<data_model::pod::Pod>>();
                states.resource.pod_type.pods = pods_of_this_type;
                states.resource.pod_type.pod_type_sel_id = pod_type_sel.id;
            } else {
