...
pub fn render(f: &mut Frame, area: Rect, states: &ui::States, store: &data_model::Store) {
    ...
    let pod_type_sel_string = if let Some(pt) = states.app.detail.pod_type_selected() {
       pt.name.to_string()
    } else { "None".to_string() };
    let pod_sel_string = if let Some(pod) = store.pod_mgr.pod_selected() {
        pod.name.to_string()
    } else { "None".to_string() };

    let text: Vec<Line> = vec![
        Line::from(format!("[Selected Account]   {account_sel}")).green(),
